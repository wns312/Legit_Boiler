  ...
  let result = MemberArray.map(
    (element, index)=>{
      ...
      return (
          ...
          <p key={index} style={{ color: "black" }}>&emsp;<img src={`http://${process.env.REACT_APP_IP_ADDRESS}:9000/${image}`} alt={image} style={{width : '50px', height : '50px', padding : '5px'}}></img>&ensp;{`${name} (${email})`}</p>
